...
function delta = symplFunc(yn, ynn, obj, z_n, z_nn, is_explicit)
    fyn = obj.retarded_func(is_explicit*yn + not(is_explicit)*ynn, is_explicit*z_n + not(is_explicit)*z_nn);
    fynn = obj.retarded_func(is_explicit*ynn + not(is_explicit)*yn, is_explicit*z_nn + not(is_explicit)*z_n);
